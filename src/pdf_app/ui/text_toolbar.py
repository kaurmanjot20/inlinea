...
class TextFormattingToolbar(Gtk.Popover):
    def __init__(self, parent_widget, annotation, store, on_update):
        ...
        # Font Family
        self.font_model = Gtk.StringList.new(["Comfortaa", "cursive", "Cantarell", "Sans", "Serif", "Monospace"])
        ...
        self.underline_btn.connect("toggled", self.on_style_toggled, 'underline')
    ...
    def on_color_changed(self, button, pspec):
        ...
        if new_color != self.annotation.color:
            self.store.record_color_change(self.annotation.id, self.annotation.color)
